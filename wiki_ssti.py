...
<div class="container">
    ...
    <ul>
        {% for p in pages %}
        <li><a href="/page/{{ p[0] }}">{{ p[0] | capitalize }}</a></li>
        {% endfor %}
    </ul>
</div>
...
def page(titulo: str):
        with db() as (_, cur):
                query = cur.execute('SELECT titulo, descricao FROM paginas WHERE titulo=?;', (titulo,))
...
if __name__ == '__main__':
        ...
        open("flag.txt", "w").write(f"$FLAG!{{{''.join(random.sample(chars, 15))}}}")
